...
// fecha más reciente de cada uno y su estado.
function AtajoPlanillaSemana({
  ...
  const filas = DIAS_LISTA.map(diaNombre => {
    ...
    const p = planillas[`${diaNombre}_${fechaKey}`];
...
function PlanillaDelDia({
  ...
  // Paso 1: confirmar envases → actualiza stock y vuelve a la planilla.
  // El envío del informe queda como Paso 2, separado, desde la planilla
  // principal (botón "Guardar y enviar informe").
  const confirmarEnvases = () => {
    ...
    ["soda", "b10", "b20"].forEach(pk => {
      ...
      const dl = llenTotal - sobrantes[pk];
  ...
  // el informe por mail. Se hace desde la vista normal de la planilla,
  // donde #planilla-capture SÍ está montado, así que no hace falta la
  // captura-previa que usaba el flujo viejo de un solo paso.
  ...
  if (mostrarCierre) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      const quedaLleno = (soderiaActual[planKeyToStockKey[pk]] || 0) + llenReal;
      const quedaVacio = (soderiaVaciosActual[planKeyToStockKey[pk]] || 0) + vacReal;

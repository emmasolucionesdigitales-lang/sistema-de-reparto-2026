...
function usarInformes({
  ...
  const enviarDiario = async (fecha, dia, imgData) => {
    ...
    const emailFinal = (() => {
      try {
        return localStorage.getItem("sr_lc_email_informes") || lic.email || "";
      } catch {
        return lic.email || "";
      }
    })();
    if (!emailFinal) {
      alert("⚠️ No hay email configurado.\n\nAndá a Config → tab Datos → Email para informes.");
      return false;
    }
    Object.assign(lic, {
      email: emailFinal
  ...
  const enviarSemanal = async fecha => {
    ...
    try {
      ...
      const lp = new Date(d);
      lp.setDate(d.getDate() - 6);
      const desde = lp.toISOString().slice(0, 10);
      const vs = (ventas || []).filter(v => v.fechaKey >= desde && v.fechaKey <= fecha && !v._esCobro && !v._esAjuste);
      const t = vs.reduce((a, v) => a + (v.neto || 0), 0);
      await window.enviarEmailBrevoRM({
        to: lic.email,
        toName: lic.negocio || "",
        subject: `📊 Semana ${desde}→${fecha} · ${fmtP(t)}`,
        htmlContent: `<div style="font-family:sans-serif;padding:20px"><h2 style="color:#7b3fc9">📊 Resumen semanal</h2><p>${desde} al ${fecha}</p><div style="font-size:28px;font-weight:700;color:#7b3fc9">${fmtP(t)}</div><p>${vs.length} entregas</p></div>`
      });
      return true;
    } catch (e) {
      return false;
  ...
  const enviarMensual = async (mes, anio) => {
    ...
    try {
      ...
      const vs = (ventas || []).filter(v => (v.fechaKey || "").startsWith(p) && !v._esCobro && !v._esAjuste);
      const t = vs.reduce((a, v) => a + (v.neto || 0), 0);
      const mn = ["", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
      await window.enviarEmailBrevoRM({
        to: lic.email,
        toName: lic.negocio || "",
        subject: `📅 ${mn[mes]} ${anio} · ${fmtP(t)}`,
        htmlContent: `<div style="font-family:sans-serif;padding:20px"><h2 style="color:#1a7a5e">📅 ${mn[mes]} ${anio}</h2><div style="font-size:28px;font-weight:700;color:#1a7a5e">${fmtP(t)}</div><p>${vs.length} entregas</p></div>`
      });
      return true;
    } catch (e) {
      return false;
...
// comparten, mismo alcance global).
...
function App() {
  ...
  React.useEffect(() => {
    if (!window.db || !window.auth || !window.auth.currentUser || !negocioId) {
      claimRef.current = Promise.resolve();
      return;
    }
    claimRef.current = window.db.collection("users").doc(negocioId).set({
  ...
  const [pantalla, setPantalla] = useState(() => {
    ...
    const savedDia = (() => {
      try {
        return JSON.parse(localStorage.getItem("sr_dia_actual") || '""');
      } catch {
        return "";
      }
    })();
  ...
  const [stockRaw, setStockRaw] = useLS("sr_stock_v4", {
    soderia: {
      sifon: 0,
      bidon10: 0,
      bidon20: 0,
    ...
    Promise.resolve(claimRef.current).then(() => cloudLoad(negocioId)).then(function (data) {
      ...
      if (data.clientes?.length) {
        ...
        const clientesLocales = (() => {
          try {
            return JSON.parse(localStorage.getItem("sr_clientes_v3") || "[]");
          } catch {
            return [];
          }
        })();
        ...
        let vistosNubeCli = [];
        try {
          vistosNubeCli = JSON.parse(localStorage.getItem("sr_clientes_vistos_nube_v1") || "[]");
        } catch {}
        ...
        try {
          localStorage.setItem("sr_clientes_vistos_nube_v1", JSON.stringify((data.clientes || []).map(c => c.id)));
        } catch {}
        ...
        try {
          localStorage.setItem("sr_clientes_v3", JSON.stringify(mergedCli));
        } catch {}
      ...
      if (data.ventas?.length) {
        ...
        const ventasLocales = (() => {
          try {
            return JSON.parse(localStorage.getItem("sr_ventas_v3") || "[]");
          } catch {
            return [];
          }
        })();
        ...
        setVentasRaw(merged);
        try {
          localStorage.setItem("sr_ventas_v3", JSON.stringify(merged));
        } catch {}
        if (cambiosLocales > 0) {
      ...
      if (data.planillas) {
        ...
        const planillasLocales = (() => {
          try {
            return JSON.parse(localStorage.getItem("sr_planillas_v1") || "{}");
          } catch {
            return {};
          }
        })();
        ...
        try {
          localStorage.setItem("sr_planillas_v1", JSON.stringify(mergedPla));
        } catch {}
      ...
      if (data.noVisitas?.length) {
        ...
        const noVisitasLocales = (() => {
          try {
            return JSON.parse(localStorage.getItem("sr_novisitas_v1") || "[]");
          } catch {
            return [];
          }
        })();
        ...
        try {
          localStorage.setItem("sr_novisitas_v1", JSON.stringify(mergedNV));
        } catch {}
      ...
      if (data.cargasDia && Object.keys(data.cargasDia).length) {
        setCargasDia(data.cargasDia);
        try {
          localStorage.setItem("sr_cargas_dia_v1", JSON.stringify(data.cargasDia));
        } catch {}
      }
      setSyncStatus("saved");
  ...
  React.useEffect(() => {
    ...
    const CHECKPOINT_KEY = "sr_minimo_mensual_facturado_hasta";
  ...
  React.useEffect(() => {
    window._descargarRespaldo = () => {
      const mantVeh = (() => {
        try {
          return JSON.parse(localStorage.getItem("sr_mant_vehiculo_v1") || "[]");
        } catch {
          return [];
        }
      })();
      const histPrecios = (() => {
        try {
          return JSON.parse(localStorage.getItem("sr_lc_hist_precios") || "[]");
        } catch {
          return [];
        }
      })();
      ...
      a.href = url;
      a.download = `respaldo-completo_reparto_${f}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    ...
    window._restaurarRespaldo = data => {
      ...
      try {
        if (data.clientes !== undefined) {
          ...
          try {
            localStorage.setItem("sr_clientes_v3", JSON.stringify(data.clientes || []));
          } catch {}
        }
        if (data.ventas !== undefined) {
          setVentasRaw(data.ventas || []);
          try {
            localStorage.setItem("sr_ventas_v3", JSON.stringify(data.ventas || []));
          } catch {}
        }
        if (data.planillas !== undefined) {
          setPlanillas(data.planillas || {});
          try {
            localStorage.setItem("sr_planillas_v1", JSON.stringify(data.planillas || {}));
          } catch {}
        }
        if (data.stock) {
          const ds = data.stock;
          ...
          setStock(ns);
          try {
            localStorage.setItem("sr_stock_v4", JSON.stringify(ns));
          } catch {}
        }
        ...
        if (data.noVisitas !== undefined) {
          setNoVisitas(data.noVisitas || []);
          try {
            localStorage.setItem("sr_novisitas_v1", JSON.stringify(data.noVisitas || []));
          } catch {}
        }
  ...
  const syncData = (overrides = {}) => {
    ...
    const data = {
      ...
      horaAvisoCierre: overrides.horaAvisoCierre || localStorage.getItem('sr_hora_notif_cierre') || '18:00',
      horasAvisoTrans: overrides.horasAvisoTrans || (() => {
        try {
          return JSON.parse(localStorage.getItem('sr_horas_notif_trans') || '["13:00","19:00"]');
        } catch {
          return ['13:00', '19:00'];
        }
      })(),
  ...
  React.useEffect(() => {
    ...
    if (huerfanas.length > 0) {
      ...
      setVentasRaw(prev => prev.filter(v => !ids.has(v.id)));
    }
  }, [ventasRaw]);
  ...
  const cerrarDia = async (fecha, dia, imgData) => {
    ...
    const licPreview = (() => {
      try {
        return JSON.parse(localStorage.getItem("sr_licencia") || "{}").email || "";
      } catch {
        return "";
      }
    })();
  ...
  React.useEffect(() => {
    ...
    const cobEfectivo = ventasDia.filter(v => v.pago === "contado" && !v._esMixtoTrans).reduce((a, v) => a + (v.pagadoNum || v.neto || 0), 0) + ventasDia.filter(v => v.pago === "mixto").reduce((a, v) => a + (Number(v.montoEfec) || 0), 0);
    const cobFiado = ventasDia.filter(v => v.pago === "fiado").reduce((a, v) => a + (v.neto || 0), 0);
    const cobTransBruto = ventasDia.filter(v => v.pago === "transferencia" && !v._esMixtoTrans).reduce((a, v) => a + (v.pagadoNum || v.neto || 0), 0) + ventasDia.filter(v => v.pago === "mixto").reduce((a, v) => a + (Number(v.montoTrans) || 0), 0);
  ...
  const licActivada = (() => {
    try {
      return JSON.parse(localStorage.getItem("sr_licencia") || "{}").activado === true;
    } catch {
      return false;
    }
  })();
  ...
  if (cargandoNube && (apiKey || binId)) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        borderRadius: 2,
        width: "50%",
        animation: "lc_slide 1.2s ease-in-out infinite alternate"
      }
    })), /*#__PURE__*/React.createElement("style", null, `@keyframes lc_slide{from{left:-50%}to{left:100%}}`));
  }

  // 1. Sin código → pedir código
  if (!apiKey || !binId) {
    return /*#__PURE__*/React.createElement(PantallaCodigoAcceso, {
      onCodigo: cod => {
        setApiKey(cod);
        setBinId(cod);
        try {
  ...
  if (!licActivada) {
    return /*#__PURE__*/React.createElement(PantallaActivacion, {
      onActivado: () => {
  ...
  if (pinGuardado && !pinOk) {
    ...
    return /*#__PURE__*/React.createElement(PantallaPINIndividual, {
      onOk: () => {
        setPantalla("portada");
        setPinOk(true);
      }
  ...
  function getDeviceIdRM() {
    ...
    if (!id) {
      ...
      localStorage.setItem('sr_device_id_push', id);
    }
    return id;
  }
  window.activarNotif = async () => {
    ...
    try {
      ...
      const token = await window.messagingLC.getToken({
        vapidKey: VAPID_PUBLIC_RM,
